...
from modules.unicode import isUnicode
...
LINE_TYPE = {"wrap": 0, "single": 1}

# Orientations
H_ORIENT = {"left": 0, "right": 1}
V_ORIENT = {"up": 0, "down": 1}

# Debug variables
DBG_BOX_PLACEMENT = {"top": 0, "bottom": 1}
DBG_BOX_INFO = {"name": 0, "textSize": 1, "boxSize": 2}
...
class TerminalTextBoxes:
    ...
    def __init__(self, userName="", charCallback=None, enterCallback=None, debug=False):
        ...
        self.__resizeTimer = threading.Timer(0.1, self.__resize_timeout)
        ...
        self.__promptLineWidth = (
            None  # Will be self.__wTerminal - (self.__promptSignSize + 1)
        )
        self.__promptString = ""
        ...
        self.__INFO_PROMPT_MIN_WIDTH = self.__infoPromptTextIndent * \
            2 + (2 + 5)
    ...
    def create_text_box(
        ...
        if width != None and width < self.__BOX_MIN_WIDTH:
            raise Exception(
                f"width is too small, must be bigger than or equal to {self.__BOX_MIN_WIDTH}"
            )
        if width != None and width < (self.__BOX_MIN_WIDTH + 2 * wTextIndent):
            raise Exception(
                f"width is too small, must be bigger than or equal to {self.__BOX_MIN_WIDTH + 2 * wTextIndent}"
            )
        ...
        if height != None and height < self.__BOX_MIN_HEIGHT:
            raise Exception(
                f"height is too small, must be bigger than or equal to {self.__BOX_MIN_HEIGHT}"
            )
        if height != None and height < (
                self.__BOX_MIN_HEIGHT + 2 * hTextIndent):
            raise Exception(
                f"height is too small, must be bigger than or equal to {self.__BOX_MIN_HEIGHT + 2 * hTextIndent}"
            )
        ...
        if not isinstance(hOrient, int) or hOrient not in H_ORIENT.values():
            raise Exception(
                "hOrient is not of integer type or not within acceptable range."
            )
        self.__boxSetup[setupName]["boxes"][boxName]["hOrient"] = hOrient

        if not isinstance(vOrient, int) or vOrient not in V_ORIENT.values():
            raise Exception(
                "vOrient is not of integer type or not within acceptable range."
            )
        ...
        if hPos != None:
            ...
            if self.__boxSetup[setupName]["boxOrder"].index(boxName) > 0:
                prevBoxIndex = self.__boxSetup[setupName]["boxOrder"].index(
                    boxName) - 1
            else:
                prevBoxIndex = 0

            self.__boxSetup[setupName]["boxes"][boxName]["hOrient"] = self.__boxSetup[
                setupName
            ]["boxes"][self.__boxSetup[setupName]["boxOrder"][prevBoxIndex]]["hOrient"]
        ...
        self.__boxSetup[setupName]["boxes"] = {
            key: self.__boxSetup[setupName]["boxes"][key]
            for key in self.__boxSetup[setupName]["boxOrder"]
        }
    ...
    def remove_text_box(self, setupName, boxName):
        ...
        if len(self.__boxSetup[setupName]["boxes"]) == 0:
            ...
            self.__boxSetup[setupName]["focusedBox"] = next(
                iter(self.__boxSetup[setupName]["boxes"])
            )  # }}}
    ...
    def set_info_prompt_text(self, text, timeout=None):
        ...
        self.__screen.addstr(
            infoPromptY, self.__infoPromptTextIndent, " ", bgAttr)
        self.__screen.addstr(infoPromptY, textStartX,
                             text[:textMaxLen], textAttr)
        self.__screen.addstr(infoPromptY, textEndX, " ", bgAttr)

        if timeout != None:
            self.__infoPromptTimer.cancel()
            self.__infoPromptTimer = threading.Timer(
                timeout // 1000, self.__info_prompt_text_timeout
            )
    ...
    def add_text_item(
        ...
        self.__boxSetup[setupName]["boxes"][boxName]["prevTextItemsLength"] = len(
            self.__boxSetup[setupName]["boxes"][boxName]["textItems"])

        self.__boxSetup[setupName]["boxes"][boxName]["textItems"].append(
            [text, attributes, LINE_TYPE[lineType]]
        )
    ...
    def update(self):
        ...
        (
            self.__hTerminal,
            self.__wTerminal,
        ) = self.__screen.getmaxyx()  # Get the terminal size
    ...
    def __update_prompt_variables(self, updateTerminal=True):
        ...
        # Edge conditions
        self.__edgeConditions.append(
            self.__wTerminal >= self.__PROMPT_MIN_WIDTH)
        self.__edgeConditions.append(
            self.__hTerminal >= self.__PROMPT_MIN_HEIGHT)
        self.__edgeConditions.append(
            self.__wTerminal >= self.__INFO_PROMPT_MIN_WIDTH)
        self.__edgeConditions.append(
            self.__hTerminal >= self.__INFO_PROMPT_MIN_HEIGHT)

        self.__updateConditionsSatisfied = all(
            condition == True for condition in self.__edgeConditions
        )
    ...
    def __update_box_variables(self, updateTerminal=True):
        ...
        nbrOfBoxes = len(
            self.__boxSetup[self.__activeBoxSetup]["boxes"]
        )  # Total amount of current boxes
        ...
        remainingUnevenWidth = wForUnfixed % (
            nbrOfWUnfixedBoxes if nbrOfWUnfixedBoxes > 0 else 1
        )
        ...
        for name, attr in self.__boxSetup[self.__activeBoxSetup]["boxes"].items():
            ...
            # Box Width/ Height
            if attr["fixedWidth"] == None:
                attr["boxWidth"] = (wForUnfixed // nbrOfWUnfixedBoxes) + (
                    1 if remainingUnevenWidth > 0 else 0
                )
                remainingUnevenWidth = (
                    (remainingUnevenWidth - 1)
                    if remainingUnevenWidth > 0 else 0)
            else:
                attr["boxWidth"] = attr["fixedWidth"]
            if attr["fixedHeight"] == None:
                attr["boxHeight"] = self.__hTerminal - (
                    self.__promptHeight + self.__FRAME_SIZE
                )
            else:
            ...
            attr["textWidth"] = (
                attr["boxWidth"] - frameTotalWidth - (attr["wTextIndent"] * 2)
            )
            frameTotalHeight = (
                self.__FRAME_SIZE * 2) + (2 if self.debug else 0)
            attr["textHeight"] = (
                attr["boxHeight"] - frameTotalHeight -
                (attr["hTextIndent"] * 2))

            # Box orientation horizontal/ vertical
            if (
                attr["hOrient"] == H_ORIENT["right"]
                and attr["fixedWidth"] != None
                and wForUnusedUsed == False
            ):
                wIndex = wIndex + wForUnused
                wForUnusedUsed = True
            if (
                attr["vOrient"] == V_ORIENT["down"]
                and attr["fixedHeight"] != None
                and hForUnusedUsed == False
            ):
                hIndex = hIndex + (
                    self.__hTerminal
                    - (self.__promptHeight + self.__FRAME_SIZE)
            ...
            attr["bottomRight"] = {
                "x": wIndex + attr["boxWidth"] - 1,
                "y": hIndex + attr["boxHeight"],
            }
            ...
            if self.debug and self.dbgBoxPlacementShow == DBG_BOX_PLACEMENT["top"]:
                attr["textStartY"] = (
                    hIndex + 2 + self.__FRAME_SIZE + attr["hTextIndent"]
                )
            else:
            ...
            if attr["visable"] == False:
                self.__edgeConditions.append(attr["boxWidth"] >= (
                    self.__BOX_MIN_WIDTH + 2 * attr["wTextIndent"]))
                self.__edgeConditions.append(
                    attr["boxHeight"]
                    >= (self.__BOX_MIN_HEIGHT + 2 * attr["hTextIndent"])
                )
        ...
        self.__promptLineWidth = self.__wTerminal - \
            (len(self.__promptSign) + 1)
    ...
    def __update_box_edge_conditions(self):
        ...
        self.__edgeConditions.append(
            ...
            >= (
                boxesMinHeight
                + self.__INFO_PROMPT_MIN_HEIGHT
                + self.__PROMPT_MIN_HEIGHT
            )
        )
    ...
    def __update_text_wrapping(self):
        ...
        for name, attr in self.__boxSetup[self.__activeBoxSetup]["boxes"].items():
            ...
            if not (
                attr["boxWidth"] != attr["prevBoxWidth"]
                or attr["boxHeight"] != attr["prevBoxHeight"]
                or len(attr["textItems"]) != attr["prevTextItemsLength"]
            ):
                continue
            ...
            for item, txtAttr, lineType in attr["textItems"]:
                if lineType == LINE_TYPE["single"]:
                    self.__boxSetup[self.__activeBoxSetup]["boxes"][name][
                        "lines"
                    ].append([item[: attr["textWidth"]], txtAttr])
                else:
    ...
    def __update_box_frames(self):
        ...
        for name, attr in self.__boxSetup[self.__activeBoxSetup]["boxes"].items():
            ...
            if (
                name == self.__boxSetup[self.__activeBoxSetup]["focusedBox"]
                and self.debug
            ):
                self.__screen.addstr(
                    boxTLY + 1, boxBRX - 1, "", curses.color_pair(CHAR_COLOR["red"])
                )
            ...
            for row in range(self.__hTerminal):
                for column in range(self.__wTerminal):
                    ...
                    if (
                        ...
                        and boxTLX == column
                        and self.debug
                        and self.dbgBoxPlacementShow == DBG_BOX_PLACEMENT["top"]
                    ):
                        self.__screen.addstr(
                            row, column, style["verticalRight"],
                            attr["frameAttr"])
                    elif (
                        (boxTLY + 2) == row
                        and boxBRX == column
                        ...
                        self.__screen.addstr(
                            row, column, style["horizontal"], attr["frameAttr"]
                        )
                    ...
                    # NORMAL BOX FRAME
                        ...
                        self.__screen.addstr(
                            row, column, style["rightDown"], attr["frameAttr"]
                        )
                    elif boxTLY == row and boxBRX == column:
                        self.__screen.addstr(
                            row, column, style["leftDown"], attr["frameAttr"]
                        )
                    elif boxBRY == row and boxTLX == column:
                        self.__screen.addstr(
                            row, column, style["rightUp"], attr["frameAttr"]
                        )
                    elif boxBRY == row and boxBRX == column:
                        self.__screen.addstr(
                            row, column, style["leftUp"], attr["frameAttr"]
                        )
                    elif (
                        (boxTLY == row or boxBRY == row)
                        and boxTLX < column
                        and boxBRX > column
                    ):
                        self.__screen.addstr(
                        ...
                        boxTLY < row
                        and boxBRY > row
                        and (boxTLX == column or boxBRX == column)
                    ):
                        self.__screen.addstr(
                            row, column, style["vertical"], attr["frameAttr"]
                        )
    ...
    def __update_prompt(self):
        ...
        for i in range(self.__promptHeight):
            self.__screen.addstr(
                self.__hTerminal - self.__promptHeight - i, 0, txt)

        displayedString = ""
        if len(self.__promptString) >= self.__promptLineWidth:
            displayedString = self.__promptString[
                self.__promptVLeftPos: self.__promptVRightPos
            ]
        else:
            displayedString = self.__promptString
        self.__screen.addstr(
            self.__hTerminal - self.__promptHeight,
            0,
            self.__promptSign + displayedString,
        )
        ...
        for name, attr in self.__boxSetup[self.__activeBoxSetup]["boxes"].items():
            ...
            if len(attr["lines"]) >= attr["textHeight"]:
                displayedText = attr["lines"][
                    -attr["textHeight"] + attr["scrollIndex"]:
                ][: attr["textHeight"]]
            else:
                displayedText = attr["lines"]
            ...
            for i, line in enumerate(displayedText):
                self.__screen.addstr(
                    attr["textStartY"] + i, attr["textStartX"],
                    line[0],
                    line[1])
    ...
    def __update_boxes_scrolls(self):
        ...
        for name, attr in self.__boxSetup[self.__activeBoxSetup]["boxes"].items():
            ...
            if len(attr["lines"]) >= attr["textHeight"]:
                ...
                boundaryEnd = (
                    attr["textStartY"] + attr["textHeight"] +
                    attr["hTextIndent"])
                ...
                startY = (
                    int(scrollBoundary * (above / len(attr["lines"]))) +
                    boundaryStart)
                endY = boundaryEnd - int(scrollBoundary *
                                         (below / len(attr["lines"])))
    ...
    def __update_visual_cursor(self):
        ...
        self.__screen.move(
            self.__hTerminal - self.__promptHeight,
            self.__promptVCursorPos + len(self.__promptSign),
        )
    ...
    def __update_boxes_frame_attr(self):
        ...
        for setupName, setupAttr in self.__boxSetup.items():
            for boxName, boxAttr in setupAttr["boxes"].items():
                boxAttr["frameAttr"] = self.__merge_attributes(
                    boxAttr["frameAttrUnmerged"]
                )
    ...
    def set_box_horizontal_orient(self, setupName, boxName, orient):
        ...
        if not isinstance(orient, int) or orient not in H_ORIENT.values():
            raise Exception(
                "orient is not of integer type or not within acceptable range."
            )
    ...
    def get_box_frame_attr(self, setupName, boxName):
        ...
        return self.__boxSetup[setupName]["boxes"][boxName][
            "frameAttrUnmerged"]
    ...
    def set_box_horizontal_pos(self, setupName, boxName, pos):
        ...
        self.__boxSetup[setupName]["boxOrder"].pop(
            self.__boxSetup[setupName]["boxOrder"].index(boxName)
        )
    ...
    def __key_handler(self, event):
        ...
        while True:
            ...
            if char == "\x00":  # "WINDOWS" KEY
                ...
                self.__resizeTimer = threading.Timer(
                    0.1, self.__resize_timeout)
            ...
            # PROMPT KEY EVENTS ---------------------------------------------------------------------------------------
                ...
                if (
                    ...
                    and self.__promptVCursorPos != self.__promptLineWidth
                ):
                    self.__promptVCursorPos += 1
                ...
                if len(self.__promptString) >= self.__promptLineWidth:
                    if (
                        len(self.__promptString) == (self.__promptVRightPos - 1)
                        and self.__promptVCursorPos != self.__promptLineWidth
                    ):
                        self.__promptVCursorPos += 1
                ...
                if (
                    self.__promptVCursorPos != 0
                    and len(self.__promptString) <= self.__promptLineWidth
                    and self.__promptVLeftPos == 0
                ):
                    self.__promptVCursorPos -= 1
                elif (
                    self.__promptVCursorPos != 0
                ...
                if (
                    self.__promptString != ""
                    and self.__promptEnterCallbackFunction != None
                ):
                    self.__promptEnterCallbackFunction(self.__promptString)
                self.__promptString = ""
                self.__promptCursorPos = 0
                self.__promptVCursorPos = 0
                self.__boxSetup[self.__activeBoxSetup]["boxes"][focusedBox][
                    "scrollIndex"
                ] = 0

            # BOX KEY EVENTS ------------------------------------------------------------------------------------------
            elif char == 259:  # <ARROW-UP> KEY (Scroll up)
                lines = self.__boxSetup[self.__activeBoxSetup]["boxes"][
                    focusedBox]["lines"]
                scrollIndex = self.__boxSetup[self.__activeBoxSetup]["boxes"][
                    focusedBox
                ]["scrollIndex"]
                textHeight = self.__boxSetup[self.__activeBoxSetup]["boxes"][
                    focusedBox
                ]["textHeight"]
                if len(lines) + scrollIndex > textHeight:
                    self.__boxSetup[self.__activeBoxSetup]["boxes"][
                        focusedBox]["scrollIndex"] -= 1
                ...
                if scrollIndex < -(len(lines) - textHeight):
                    self.__boxSetup[self.__activeBoxSetup]["boxes"][
                        focusedBox]["scrollIndex"] = -(len(lines) - textHeight)
    ...
    def __reset_info_prompt(self):
        ...
        #     startOfInfoPromptY,
        #     startOfInfoPromptX,
        #     self.__wTerminal * self.__infoPromptChar,
        #     bg,
        # )
